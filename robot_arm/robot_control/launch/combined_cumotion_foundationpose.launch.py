...
def generate_launch_description():
    ...
    xrdf_path = os.path.join(
        ...
        'combined_urdf', 'combined_robot.xrdf'
    )
    urdf_path = os.path.join(
        get_package_share_directory('robot_control'),
        'combined_urdf', 'combined_robot.urdf'
    )
   
    urdf_cumotion_path = os.path.join(
        get_package_share_directory('robot_control'),
        'combined_urdf', 'arm5dof.urdf'
    )
	

    moveit_config = (
        MoveItConfigsBuilder("combined_robot", package_name='final_combined_moveit_config')
    ...
    rviz_config_file = os.path.join(
        get_package_share_directory('final_combined_moveit_config'),
        'config','moveit.rviz',
    )
    ...
    foundation_pose = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory('robot_control'),
                'launch',
                'foundationpose_yolov8_realsense.launch.py'
            )
       )
    )

    cumotion_pick_node = Node(
        package='robot_control',
        executable='cumotion_pick_node',
        name='cumotion_pick_node',
        output='screen',
    )
