...
class UartServoManager:
	...
	def set_servo_angle(self, servo_id:int, angle:float, is_mturn:bool=False, interval:float=None, velocity:float=None, t_acc:int=20, t_dec:int=20,  power:int=0, mean_dps:float=100.0):
		...
		if interval is not None and interval >= 200:
			# 指定周期
			# 判断周期设定是否合法
			...
			if is_mturn:
				...
				#print(f"interval {interval} interval if")
			...
			if is_mturn:
				...
				param_bytes = struct.pack('<BhHH', servo_id, angle, velocity, power)
			...
			if is_mturn:
				...
				#print(f"interval {interval} else if")
				interval = int(interval)             
